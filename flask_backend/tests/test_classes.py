...
def test_admin_can_enroll_students_via_csv_for_other_teachers_course(test_client, make_admin):
    """
    GIVEN a course owned by a teacher
    WHEN an admin calls POST /class/enroll_students with a CSV
    THEN enrollment should succeed (admin override)
    """
    teacher = User(
        name="Teacher",
        email="t@example.com",
        hash_pass=generate_password_hash("tpass"),
        role="teacher",
    )
    ...
    response = test_client.post(
        "/class/enroll_students",
        data=json.dumps({"class_id": course.id, "students": csv_text}),
        headers={"Content-Type": "application/json"},
    )
...
def test_enroll_in_class_unauthorized(test_client):
    """
...
def test_search_classes_by_name(test_client, make_admin, enroll_user_in_course):
    """
    GIVEN a created class
    WHEN a student searches by a partial name
    THEN an enrolled class should appear in the results with metadata
    """
...
def test_search_classes_student_cannot_see_unenrolled_classes(test_client, make_admin):
    """
    GIVEN a class that the student is not enrolled in
    WHEN the student searches by class name
    THEN the class should not appear in search results
    """
    make_admin(email="teacher@example.com", password="teacher", name="teacheruser")
    test_client.post(
        "/auth/login",
    ...
    resp = test_client.get("/class/search?q=Physics", headers={"Content-Type": "application/json"})
    assert resp.status_code == 200
    data = resp.json
    assert "results" in data
    assert data["results"] == []
